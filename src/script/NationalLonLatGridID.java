	...
	public static void main(String[] args){
		List<String> paths=FileUtil.getAbsolutePathFromDIR(Config.NATIONAL_BOUNDARY_DIR);
			...
			for(String path : paths){
				...
				File writer_file=new File(writer_file_path.replace(reader_file.getName(), ""));
				if(!writer_file.exists())
					writer_file.mkdirs();
				BufferedWriter writer=new BufferedWriter(new FileWriter(writer_file_path));
				String line;
					...
					for(String lonlat_str : line_array){
						...
						if(lonlat_str_array.length==2){
							...
							for(int i=0;i<lonlat_str_array2.length;i+=2){
								LonLat lonLat=new LonLat(lonlat_str_array2[i],lonlat_str_array2[i+1]);  //此处需要注意，具体的分割符是什么
								lonLat.baiduTOgcj();
								GeographicGrid grid=new GeographicGrid(lonLat);
								writer.write(grid.toStringLonLatGeographicCode()+"\r\n");
							}
